...
class ModelArguments:
    ...
    quantize: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to use bayesian bits or not."},
    )
    quantization_lmbd: Optional[float] = field(
        default=0.0,
        metadata={"help": "Coefficient for gate loss"},
    )
    learn_gates: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to learn quantization gates."},
    )
    learn_scales: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to learn scales."},
    )
    gamma_4_init: Optional[float] = field(
        default=6.0,
        metadata={"help": "Inital gate 4 value."},
    )
    gamma_8_init: Optional[float] = field(
        default=6.0,
        metadata={"help": "Inital gate 8 value."},
    )
    gamma_16_init: Optional[float] = field(
        default=6.0,
        metadata={"help": "Inital gate 16 value."},
    )
    gamma_32_init: Optional[float] = field(
        default=6.0,
        metadata={"help": "Inital gate 32 value."},
    )
    gates_lr: Optional[float] = field(
        default=1e-2,
        metadata={"help": "Learning rate for gates."},
    )
    scales_lr: Optional[float] = field(
        default=1e-2,
        metadata={"help": "Learning rate for scales."},
    )
    scales_type: Optional[str] = field(
        default="range",
        metadata={"help": "Scaling type."},
    )
    prune_rank: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to reduce rank."},
    )
    ...
    warmup_epochs: Optional[int] = field(
        default=2,
        metadata={"help": "Number of epochs for which only LoRA finetuned, without quantization."},
    )
...
def main():
    ...
    print("use_deterministic_algorithms: " + str(torch.are_deterministic_algorithms_enabled()))

    if model_args.use_wandb:
        wandb.init(
            project=f"{data_args.task_name}_{model_args.model_name_or_path.split('/')[-1]}_{datetime.now().strftime('%m_%d_%H')}")
        wandb.config = dict(
            experiment=data_args.task_name + datetime.now().strftime('%m_%d_%H'),
    ...
    if training_args.do_train:
        ...
        if model_args.quantize:
            dummy_input_ids = torch.tensor([train_dataset[index]['input_ids']], device=device)
            if "token_type_ids" in train_dataset[index].keys():
                dummy_token_type_ids = torch.tensor([train_dataset[index]["token_type_ids"]], device=device)
            else:
                dummy_token_type_ids = None
    ...
    if training_args.do_train:
        ...
        if last_checkpoint is not None:
            ...
            # checkpoint.
            if model_args.resume_training and AutoConfig.from_pretrained(
