...
class BayesianBitsQuantizer(nn.Module):
    ...
    def get_gates(self, N=1):
        ...
        if self.prune_rank:
            probs = gate_f(self.gamma_2, *args, **kwargs) if self.prune_rank else None
    ...
    def regularizer(self):
        ...
        kl_b2 = self.get_bops(2) * q_b2 if self.include_pruning or self.prune_rank else ones
        ...
        reg_i = reg_i / (self.gamma_2.size(0) if self.include_pruning or self.prune_rank else 1.0)
        return reg_i
    ...
    def forward(self, x):
        ...
        if (self.prune_rank or self.include_pruning) and self.gamma_2 is None and not self.act_quant:
            ...
            gamma_2 = torch.Tensor((x.size(0) - 1) * [6.0])
